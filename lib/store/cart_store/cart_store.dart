...
abstract class _CartStore with Store {
  ...
  @action
  Future<void> createItemOrder(CreateOrderDto input, BuildContext context,
      String token, bool isFromQuickOrder) async {
    if (createdOrderState == LoadingStatusEnum.loading) {
      return;
    }
    createdOrderState = LoadingStatusEnum.loading;

    dynamic _resp = await _orderProvider.createOrders(
        input: input, token: token, isFromQuickOrder);
